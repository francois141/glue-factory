...
class ScaleLSD(BaseModel):
    ...
    def cli(cls, parser):
        try:
            ...
            parser.add_argument(
                ...
                default=0.008,
                type=float,
                help="junction threshold heatmap",
            )
        except:
            pass
    ...
    def wireframe_matcher(self, juncs_pred, lines_pred, hat_points, is_train=False):
        ...
        global_idx = (
            idx_lines_for_junctions[:, 0] * juncs_pred.shape[0]
            + idx_lines_for_junctions[:, 1]
        )
    ...
    def detect_junctions(self, images, junction_heatmaps=None):
        ...
        for i in range(batch_size):
            ...
            junctions, scores = self.get_junctions(
                jloc_pred_nms,
                joff_pred[i],
                topk=self.num_junctions_inference,
                th=self.junction_threshold_hm,
            )
    ...
    def compute_hatlines(self, images):
        ...
        lines_pred_batch, hat_points_batch = self.hafm_decoding(
            md_pred, dis_pred, None, flatten=True, return_points=True
        )
    ...
    def forward_test(self, images, annotations=None, merge=False):
        ...
        scale_x_ajustment = images.shape[2] / 512
        scale_y_ajustment = images.shape[3] / 512
        ...
        if annotations["use_lsd"]:
            raise NotImplementedError("TODO: Implement part with use_lsd")
        ...
        output_list = []
        graph_pred = torch.zeros(
            (batch_size, self.num_junctions_inference, self.num_junctions_inference),
            device=images.device,
        )
        for i in range(batch_size):
            if annotations["use_nms"]:
                ...
                jloc_pred_nms = self.non_maximum_suppression(
                    jloc_pred[i], kernel_size=1
                )
            topK = min(
                self.num_junctions_inference,
                int((jloc_pred_nms > self.junction_threshold_hm).float().sum().item()),
            )
            juncs_pred, juncs_score = self.get_junctions(
                jloc_pred_nms, joff_pred[i], topk=topK, th=self.junction_threshold_hm
            )

            lines_adjusted, indices_adj, supports, hat_points, counts = (
                self.wireframe_matcher(
                    juncs_pred, lines_pred_batch[i], hat_points_batch[i]
                )
            )
            ...
            lines[:, 0] *= scale_y_ajustment
            lines[:, 2] *= scale_y_ajustment
            lines[:, 1] *= scale_x_ajustment
            lines[:, 3] *= scale_x_ajustment
