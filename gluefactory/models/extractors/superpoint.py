...
def sample_descriptors_fix_sampling(keypoints, descriptors, s: int = 8, normalize=True):
...
class SuperPoint(BaseModel):
    ...
    def _forward(self, data):
        ...
        if self.conf.sparse_outputs:
            ...
            best_kp = remove_borders(best_kp, self.conf.remove_borders, h * 8, w * 8)
            ...
            if (len(keypoints) == 1) or self.conf.force_num_keypoints:
                ...
                if self.conf.legacy_sampling:
                    ...
                    desc = sample_descriptors_fix_sampling(keypoints, all_desc, 8)
            else:
                if self.conf.legacy_sampling:
                    desc = [
    ...
    def homography_adaptation(self, img):
                ...
                Hs.append(
                    torch.tensor(
                        sample_homography_corners(
                            (w, h), patch_shape=(w, h), **self.conf.ha.H_params
                        )[0],
                        dtype=torch.float,
                        device=device,
                    )
        ...
        for i in range(n_mini_batch):
            ...
            warped_imgs = warp_perspective(
                torch.repeat_interleave(img, nh, dim=0), H, (h, w), mode="bilinear"
            )
            ...
            with torch.no_grad():
                ...
                count = warp_perspective(
                    torch.ones_like(score).unsqueeze(1), H, (h, w), mode="nearest"
                )
                count = erosion(count, self.erosion_kernel)
                count = warp_perspective(count, H_inv, (h, w), mode="nearest")[:, 0]

                # Warp back the scores
                score = warp_perspective(
                    score[:, None], H_inv, (h, w), mode="bilinear"
                )[:, 0]
