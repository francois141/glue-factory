...
if __name__ == "__main__":
    ...
    for it, data in enumerate(train_loader):
        ...
        # Load distance field
        img_np = data["deeplsd_distance_field"].permute(1, 2, 0).numpy()
        img_np = img_np[:, :, 0]
        img_np = nicer_display(img_np) * 255
        dataset["deeplsd_distance_field"] = -img_np

        # Load angle field
        img_np = data["deeplsd_angle_field"].permute(1, 2, 0).numpy()
        img_np = img_np[:, :, 0]
        dataset["deeplsd_angle_field"] = img_np

        # Load superpoint Heatmap
        img_np = data["superpoint_heatmap"].permute(1, 2, 0).numpy()
