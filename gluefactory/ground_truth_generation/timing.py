...
def run_measurement(
    dataloader, model, num_s, name, device, batch_size, do_jpldd_inner_timings=False
):
    count = 0
    timings = []
    for img in tqdm(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "config", type=str, choices=["aliked", "sp", "deeplsd", "jpldd"]
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--batch_size", type=int, default=1, help="Batch size used for the model"
    )
