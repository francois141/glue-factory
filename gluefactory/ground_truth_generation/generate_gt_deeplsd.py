...
def generate_ground_truth_with_homography_adaptation(
    img, net: DeepLSD, num_H=100, H_params=default_H_params, aggregation="median", bs=1
):
    ...
    for i in range(num_H):
        if i == 0:
            ...
            Hs.append(
                torch.tensor(
                    sample_homography_deeplsd((h, w), **H_params),
                    dtype=torch.float,
                    device=device,
                )
            )
    ...
    for i in range(n_mini_batch):
        ...
        with torch.no_grad():
            ...
            df, angle, offset, count = warp_afm(
                outs["df"], outs["line_level"], outs["offset"], torch.inverse(H)
            )
            del outs
    ...
    circ_bound = (torch.min(np.pi - angles, angles) * counts).sum(0) / counts.sum(
        0
    ) < 0.3
...
def warp_afm(df, angle, offset, H):
    ...
    scaling = torch.norm(warped_closest - warped_pix_loc, dim=-1) / offset_norm
    scaling[zero_offset] = 0
    warped_closest = warp_perspective(
        warped_closest.permute(0, 3, 1, 2), H, (h, w), mode="nearest"
    ).permute(0, 2, 3, 1)
    warped_offset = warped_closest - pix_loc

    # Warp the DF
    warped_df = warp_perspective(df.unsqueeze(1), H, (h, w), mode="bilinear")[:, 0]
    warped_scaling = warp_perspective(scaling.unsqueeze(1), H, (h, w), mode="bilinear")[
        :, 0
    ]
    warped_df *= warped_scaling

    # Warp the angle
    closest = pix_loc + torch.stack([torch.sin(angle), torch.cos(angle)], dim=-1)
    warped_closest = warp_points(closest, H)
    warped_angle = torch.remainder(
        torch.atan2(
    ...
    warped_angle = warp_perspective(
        warped_angle.unsqueeze(1), H, (h, w), mode="nearest"
    )[:, 0]

    # Compute the counts of valid pixels
    H_inv = torch.inverse(H)
    counts = warp_perspective(
        torch.ones_like(df).unsqueeze(1), H_inv, (h, w), mode="nearest"
    )
