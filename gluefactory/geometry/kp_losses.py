...
def get_pos_focal_loc_loss(
    scores, scores_w, loc, loc_w, keypoints, valid_kp, H, gamma, max_dist=8, k=8
):
    ...
    pos_cells_w[~valid] = 0
    pos_scores_w = scores_w[bs_grid, pos_cells_w[:, :, 0], pos_cells_w[:, :, 1]]
...
def soft_argmax_loss(
    scores, scores_w, keypoints, valid_kp, H, gamma, radius, max_dist=8
):
    bs, h, w = scores.shape
    n_kp = keypoints.shape[1]
    width = 2 * radius + 1
...
def soft_argmax_peaky_loss(
    scores, scores_w, keypoints, valid_kp, H, radius, max_dist=8
):
    def get_peaky_loss(scores, kp_rd, kp_ref, valid_kp, radius):
        ...
        unfolded_scores = F.unfold(scores[:, None], (width, width), padding=radius)
        ...
        pixel_coords = unfolded_grid[kp_rd[:, :, 0], kp_rd[:, :, 1]].reshape(
            bs, n_kp, 2, width * width
        )
        ...
        peaky_loss = (peaky_loss * valid_kp).sum(dim=1) / (valid_kp.sum(dim=1) + eps)
        return peaky_loss
    ...
    pred_kp = torch.stack(soft_argmax_refinement(keypoints_rd, scores, radius), dim=0)
    peaky_loss = get_peaky_loss(scores, keypoints_rd, pred_kp, valid_kp, radius)

    # Peaky loss in the second image
    pred_kp_w = torch.stack(
        soft_argmax_refinement(keypoints_rd_w, scores_w, radius), dim=0
    )
    peaky_loss_w = get_peaky_loss(
        scores_w, keypoints_rd_w, pred_kp_w, valid_kp * valid.float(), radius
    )
...
def alike_loss(
    scores, scores_w, pred_kp, pred_kp_w, sp_kp, valid_kp, H, gamma, radius, max_dist=8
):
    bs, h, w = scores.shape
    ...
    # Location loss
    warped_pred_kp = warp_points(pred_kp[:, :, [1, 0]], H)[:, :, [1, 0]]
    warped_pred_kp_w = warp_points(pred_kp_w[:, :, [1, 0]], torch.inverse(H))[
        :, :, [1, 0]
    ]
    dist_w = torch.norm(warped_pred_kp[:, :, None] - pred_kp_w[:, None], dim=3).min(
        dim=1
    )[0]
    dist = torch.norm(warped_pred_kp_w[:, None] - pred_kp[:, :, None], dim=3).min(
        dim=2
    )[0]
...
def sargmax_ms_loss(
    ...
    if with_loc:
        ...
        pred_kp = torch.stack(
            soft_argmax_refinement(keypoints_rd, heatmap, radius), dim=0
        )
        warped_pred_kp = warp_points(pred_kp, H)
        pred_kp_w = torch.stack(
            soft_argmax_refinement(keypoints_rd_w, heatmap_w, radius), dim=0
        )
        dist = torch.norm(warped_pred_kp - pred_kp_w, dim=2)
        valid = valid * (dist < max_dist).float()  # Ignore faraway kp
        loc_loss = (dist * valid).sum(dim=1) / (valid.sum(dim=1) + eps)
...
def kp_ce_loss(logits, gt_kp, valid_kp, valid_mask=None):
    ...
    gt_kp_int = torch.clamp(
        torch.round(gt_kp).long(),
    ...
    gt_scores = torch.cat(
        [2 * gt_scores, torch.ones_like(gt_scores[:, :1])], dim=1
    )  # gt_scores is now [B, 65, H, W]
    gt_labels = torch.argmax(
        gt_scores + torch.rand_like(gt_scores) * 0.1, dim=1
    )  # breaks ties for kp in the same cell
    # gt_labels is [B, H, W]
