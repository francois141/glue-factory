...
def parse_file(path):
    ...
    with open(path, "r") as f:
        for line in f:
            ...
            if "loc_error@" in line or "mH_err@" in line or "repeatability@" in line:
                parts = line.split(":")
                key = parts[0].strip().lstrip("{")
                val = parts[1].strip().rstrip(",}; ")
                results[key] = float(val)

            elif line.startswith("mloc_error"):
                val = line.split(":")[1].strip().rstrip(",}; ")
                results["mloc_error"] = float(val)

            elif line.startswith("mnum_lines"):
                val = line.split(":")[1].strip().rstrip(",}; ")
                results["mnum_lines"] = float(val)

            elif line.startswith("mrepeatability"):
                val = line.split(":")[1].strip().rstrip(",}; ")
                results["mrepeatability"] = float(val)
